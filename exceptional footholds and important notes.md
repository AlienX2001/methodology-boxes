...
## multimaster
### notes
1. If we get a lots of 403's from dirbusting, or from attempting a sqli, or any type of web attack, then this means that there is a WAF in place.
2. To find bypasses of the WAF in case of sqli, we can use seclists' /usr/share/seclists/Fuzzing/special-chars.txt list to fuzz for special chars. We can use burp community's intruder since its slow enough to evade the WAF for timing based firewalling, a 403 means it is not allowed, a 500 or a 200 means it is allowed
3. If we find that \ is allowed by the WAF but ' isnt, then we can use the charunicodeescape tamper scriptl using the following `sqlmap --tamper="charunicodeescape" AND THEN OTHER OPTIONS`, and you might want to specify a delay with `--delay=NumOfSeconds` in between requests so as to evade timing based firewalling.
